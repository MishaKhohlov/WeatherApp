...
//		axis: 'x',
    ...
    function customScroll($container, options) {
        ...
        /*------------------------search HEIGHT--------------------------*/
        var menu_set_cities = $('.set_cities');
        ...
        var custom_scroll_inner = $('.custom-scroll_inner');
        var heightScrollContent = custom_scroll_inner[0].childElementCount*$('.set_cities__town').outerHeight(true);
        var bar = data.$bar;

        $(window).resize(function () {
            custom_scroll_inner.css('maxHeight', menu_set_cities.height());
            if(menu_set_cities.height() >= heightScrollContent) {
                bar.hide();
            } else {
                bar.show();
            }
